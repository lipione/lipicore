...
7. **LLM Engine:** Local vLLM model servers exposed through OpenAI-compatible endpoints. The repository presents model routes as LipiFast and LipiCore; the current production server routes all text lanes through LipiCore. OCR extraction uses open-source Tesseract, while LipiCore can provide separate document-image review where enabled.
